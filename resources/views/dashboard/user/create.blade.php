...
@section('content')
    <div class="mx-auto max-w-2xl">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="font-bold text-gray-800 text-2xl">Tambah User</h1>
                <p class="text-gray-600">Tambahkan user baru.</p>
            </div>
        ...
        <div class="bg-white shadow-sm border border-gray-200 rounded-xl overflow-hidden">
            <form action="{{ route('user.store') }}" method="POST" class="space-y-4 p-6">
                @csrf

                <!-- Nama -->
                <div>
                    <label for="nama" class="block mb-1 font-medium text-gray-700 text-sm">Nama</label>
                ...
                <!-- Roles -->
                <div>
                    <label class="block mb-2 font-medium text-gray-700 text-sm">Roles</label>
                    <div class="space-y-2">
                        @foreach ($roles as $role)
                            <label class="flex items-center">
                                <input type="checkbox" name="roles[]" value="{{ $role->idrole }}"
                                    class="border-gray-300 rounded focus:ring-cyan-500 text-cyan-600"
                                    {{ in_array($role->idrole, old('roles', [])) ? 'checked' : '' }}>
                                <span class="ml-2 text-gray-700 text-sm">{{ $role->nama_role }}</span>
                            </label>
                        @endforeach
                    </div>
                    @error('roles')
