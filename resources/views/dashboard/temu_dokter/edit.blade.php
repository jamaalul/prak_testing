...
@section('title', 'Edit Temu Dokter - RSHP UNAIR')

@section('content')
    <div class="mx-auto max-w-2xl">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="font-bold text-gray-800 text-2xl">Edit Reservasi Temu Dokter</h1>
                <p class="text-gray-600">Perbarui status dan informasi reservasi.</p>
            </div>
        ...
        <div class="bg-white shadow-sm border border-gray-200 rounded-xl overflow-hidden">
            <form action="{{ route('temu-dokter.update', $temuDokter->idreservasi_dokter) }}" method="POST"
                ...
                <div class="space-y-4">
                    <!-- Info Statis -->
                    <div class="gap-4 grid grid-cols-2 bg-gray-50 p-4 border border-gray-100 rounded-lg">
                        <div>
                            <p class="font-semibold text-gray-500 text-xs uppercase">No. Urut</p>
                            <p class="font-bold text-cyan-700 text-lg">#{{ $temuDokter->no_urut }}</p>
                        </div>
                        <div>
                            <p class="font-semibold text-gray-500 text-xs uppercase">Waktu Daftar</p>
                            <p class="font-medium text-gray-700">{{ $temuDokter->waktu_daftar }}</p>
                        </div>
                    </div>
                        ...
                        <select name="dokter_pemeriksa" id="dokter_pemeriksa"
                            ...
                            @foreach ($dokters as $dokter)
                                <option value="{{ $dokter->iduser }}"
                                    {{ old('dokter_pemeriksa', $temuDokter->roleUser->iduser) == $dokter->iduser ? 'selected' : '' }}>
                                    {{ $dokter->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="status" id="status"
                            ...
                            <option value="0" {{ old('status', $temuDokter->status) == 0 ? 'selected' : '' }}>
                                Mendatang</option>
                            <option value="1" {{ old('status', $temuDokter->status) == 1 ? 'selected' : '' }}>Selesai
                            </option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-red-500 text-sm">{{ $message }}</p>
