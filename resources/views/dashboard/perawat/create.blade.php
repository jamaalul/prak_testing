...
@section('title', 'Tambah Perawat - RSHP UNAIR')

@section('content')
    <div class="mx-auto max-w-2xl">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="font-bold text-gray-800 text-2xl">Tambah Perawat</h1>
                <p class="text-gray-600">Tambahkan data perawat baru.</p>
            </div>
        ...
        <div class="bg-white shadow-sm border border-gray-200 rounded-xl overflow-hidden">
            <form action="{{ route('perawat.store') }}" method="POST" class="space-y-4 p-6">
                @csrf

                <!-- Nama -->
                <div>
                    <label for="nama" class="block mb-1 font-medium text-gray-700 text-sm">Nama Perawat</label>
                ...
                <!-- Pendidikan -->
                <div>
                    <label for="pendidikan" class="block mb-1 font-medium text-gray-700 text-sm">Pendidikan</label>
                    <input type="text" name="pendidikan" id="pendidikan" value="{{ old('pendidikan') }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all outline-none @error('pendidikan') border-red-500 @enderror"
                        required>
                    @error('pendidikan')
                        <p class="mt-1 text-red-500 text-sm">{{ $message }}</p>
                    @enderror
                ...
                <!-- No HP -->
                <div>
                    <label for="no_hp" class="block mb-1 font-medium text-gray-700 text-sm">Nomor HP</label>
