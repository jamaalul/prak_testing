...
<div class="max-w-xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Edit Ras Hewan</h1>
            <p class="text-gray-600">Perbarui informasi ras dan hubungkan dengan jenis hewan.</p>
        </div>
        <a href="{{ route('ras-hewan.index') }}" class="flex items-center text-sm text-gray-500 hover:text-gray-700 transition-colors">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <form action="{{ route('ras-hewan.update', $rasHewan->idras_hewan) }}" method="POST" class="p-6 space-y-4">
            @csrf
            @method('PUT')

            <!-- Nama Ras -->
            <div>
                <label for="nama_ras" class="block text-sm font-medium text-gray-700 mb-1">Nama Ras</label>
            ...
            <!-- Jenis Hewan -->
            <div>
                <label for="idjenis_hewan" class="block text-sm font-medium text-gray-700 mb-1">Jenis Hewan</label>
